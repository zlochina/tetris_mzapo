...
void draw_block(int8_t x, int8_t y, application_t *app, unsigned short color,
  ...
  // convert coords
  if (!is_next_section) {
    x_new = app->game.tetris_section.x + 20 * x + 19;
    y_new = app->game.tetris_section.y + 20 * y;
  } else {
    x_new = app->game.next_section.x + 20 * x + 29;
...
void game_state(application_t *app) {
  ...
  // init game variables (game_field, so on)
  init_game(app);

  /*
// debug
// figure_t fig = get_random_figure(app->game.figure_arr);
figure_t fig = app->game.figure_arr.S;
test_animation(fig, *app);
...
void init_game(application_t *app) {
  figure_arr_t fig_arr = app->game.figure_arr;
...
void update_state(application_t *app, uint8_t flags) {
  ...
  if (flags & 0x1) {  // FALL_A_ROW
    ...
    figure_position_t new_figure_pos;
    new_figure_pos.figure_state = current_figure_pos.figure_state;
    new_figure_pos.figure = current_figure_pos.figure;
    new_figure_pos.x = current_figure_pos.x;
    new_figure_pos.y = current_figure_pos.y + 1;
  ...
  if (flags & 0x2) {  // LEFT_ORIENTATION
    ...
    // but when the problem is side walls, figure should rotate freely (in this
    // case figure will be just moved a little) draw_figure
    return;
  }
...
_Bool is_possible_figure_position(figure_position_t new_fig_pos,
  ...
  int left_shift = 9 - new_fig_pos.x;
  int start_row = new_fig_pos.y;

  for (int i = 0; i < 4; i++) {
    if (start_row + i < 0) continue;

    uint16_t cur_field_row = game_field.field[start_row + i];
    uint16_t cur_fig_row =
        new_fig_pos.figure.fig[new_fig_pos.figure_state * 4 + i] << left_shift;
...
void update_game_field_array(application_t *app) {
  ...
  int left_shift = 9 - cur_fig_pos.x;
  int start_row = cur_fig_pos.y;

  for (int i = 0; i < 4; i++) {
    uint16_t cur_field_row = app->game.game_field.field[start_row + i];
    uint16_t cur_fig_row =
        cur_fig_pos.figure.fig[cur_fig_pos.figure_state * 4 + i] << left_shift;
