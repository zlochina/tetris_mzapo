...
#define LIGHT_RED 0xF08080
#define LIGHTER_RED 0xFFDAB9
unsigned int compress_rgb_color(unsigned color) {
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < 320; i++) {
    for (j = 0; j < 480; j++) {
      if (i % 4 || i < 100 && j < 100) {
        parlcd_write_data(parlcd_mem_base, compress_rgb_color(LIGHT_RED));
      } else {
        parlcd_write_data(parlcd_mem_base, compress_rgb_color(LIGHTER_RED));
      }
    }
  }
  printf("done\n");
